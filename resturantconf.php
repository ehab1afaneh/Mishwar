...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO places (place_id ,type,name,address,price,photo,lat,lng) 
  			  VALUES ('$place_id','restaurant','$place_name','$address','$price','$photo','$lat','$lng')";
    ...
    if($type == 'f') {
        ...
        $sql = "INSERT INTO booked_item (id,place_id ,user_email,type) 
  			  VALUES ('$id','$place_id','$email','restaurant')";
    }
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        if ($count == 0) {
            ...
            if ($conn->query($sql) === TRUE) {
                if ($type == 'b') {
                    header("Location: resturantphp.php?status=1#cont");
                } else {
                    header("Location: resturantphp.php?status=3#cont");
                }
                echo 'new';
            } else {
                if ($type == 'b') {
                    header("Location: resturantphp.php?status=2#cont");
                } else {
                    header("Location: resturantphp.php?status=4#cont");
                }
            }
